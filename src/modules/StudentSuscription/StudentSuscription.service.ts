...
class StudentSuscriptionServices {
    ...
    async findAll(){
        try {
            ...
            throw new Error(`Error al buscar todas los studentSuscriptions: ${error}`)
        }
    }

    async findOne(idStudent: number, idSuscription: number){
        try {
            return await StudentSuscriptionRepository.findOne(idStudent, idSuscription)
        } catch (error) {
    ...
    async findByIdStudent(idStudent: number){
        try {
            const student = await StudentSuscriptionRepository.findByIdStudent(idStudent);
            if(student.length === 0){
                return null;
            }
