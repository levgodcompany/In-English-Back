...
class SuscriptionService {
  ...
  async assignBenefitToSuscription(idSuscription: number, idBenefit: number) {
    try {
      return SuscriptionRepository.assignBenefitToSuscription(
        idSuscription,
        idBenefit
      );
    } catch (error) {
  ...
  async assignPaymentMethodToSuscription(
    ...
    try {
      return SuscriptionRepository.assignPaymentMethodToSuscription(
        idSuscription,
        idPaymentMethod
      );
    } catch (error) {
  ...
  async assignPaymentMethodsToSuscription(
    ...
    try {
      ...
      for (const { id } of paymentMethods) {
        await SuscriptionRepository.assignPaymentMethodToSuscription(
          idSuscription,
          id
        );
      }
  ...
  async assignStudentToSuscription(idSuscription: number, idStudent: number) {
    try {
      return SuscriptionRepository.assignStudentToSuscription(
        idSuscription,
        idStudent
      );
    } catch (error) {
